...
const TextSummarizer = ({typewriterStorage}) => {
    const textRef = useRef(null);
    const textRef2 = useRef(null);

    useTextAnimation([textRef, textRef2]);
    useTypewriterEffect([textRef, textRef2], [100, 10], "text_summarizer", typewriterStorage);
            ...
            <div className="text-justify py-1-12 mx-1/7 space-y-16">
                <div ref={textRef}>
                    <h1 className="text-center font-bold typewriter">
                        {`ML-based text summarizer`.split(" ").map((word, i) => <span key={i} className="animated">{word} </span>)}
                    </h1>
                    <div className="pt-16">
